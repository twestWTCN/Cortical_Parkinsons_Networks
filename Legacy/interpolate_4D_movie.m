...
nr = 1; subname = {'DS'}; i = 1; ref_chan = 'STN_R23';

load('C:\Users\Tim\Documents\Work\Cortical_Networks\tmp_files\cSource_DS.mat')
gSource = cSource;
...
N = 2;
xq = 1:0.5:size(gSource,2); %linspace(1,16,16*N);
yq = 1:0.5:size(gSource,3); %linspace(1,13,13*N);
zq = 1:0.5:size(gSource,4); %linspace(1,12,12*N);
tq = 1:0.2:size(gSource,1); %linspace(1,125,125*N);
...
for j = 1:nframes
    ...
    subplot(2,2,1)
    scatter3(xr,yr,zr,(Vr.^2).*800,Vr);
    view([0 90]); caxis([-0.2 0.5]);
    
    subplot(2,2,2)
    scatter3(xr,yr,zr,(Vr.^2).*800,Vr);
    view([90 0]); caxis([-0.2 0.5]);
    ...
    subplot(2,2,4)
    scatter3(xr,yr,zr,(Vr.^2).*400,Vr);
    view([30 30]); caxis([0 0.5]);
...
save([datapathr subname{i} '\ftdata\granger_movie_' num2str(nr)],'M','-v7.3')
