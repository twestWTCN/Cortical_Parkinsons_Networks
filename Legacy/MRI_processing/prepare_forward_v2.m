function prepare_forward_v2(datapathr,subname,steps,fresh)
...
%                 system(['C:\Users\Tim\Documents\ROBEX\runROBEX.bat ' datapathr subname{i} '\MRI\orig\' mriname ' ' datapathr subname{i} '\MRI\r' subname{i} '_ss.nii'])
                system(['C:\Users\Tim\Documents\ROBEX\runROBEX.bat ' datapathr subname{i} '\MRI\r' subname{i} '_ra.nii'  ' ' datapathr subname{i} '\MRI\r' subname{i} '_ss.nii'])
            case 'segment'
                %% Segmentations
                ...
                mri = ft_read_mri([datapathr subname{i} '\MRI\r' subname{i} '_ss_bc.nii'],'dataformat','nifti_spm');
                
                % get segmentation
                mri.brain  = mri.anatomy==2;
                %                 mri = rmfield(mri,'anatomy');
                save([datapathr subname{i} '\ftdata\r' subname{i} '_bc_seg'],'mri','-v7.3')
                
            case 'reslice'
                load([datapathr subname{i} '\ftdata\r' subname{i} '_bc_seg'])
                %% Reslice
                cfg = [];
                cfg.resolution = 1;
                cfg.dim = [256 256 256];
