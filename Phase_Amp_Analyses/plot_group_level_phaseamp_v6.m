function plot_group_level_phaseamp_v6(R)
if nargin<1
...
%Phase Amp Analy
for breg = 1:numel(R.bregname);
    load([R.resultspathr '\Group\seganalysis\pdense_' R.bregname{breg}],'densesave')
    
    for k = 1:5 %5 for granger
        i = 0; Ai = 0; Bi = 0; A = []; B = []; Ag = []; Bg = [];
        % WITH NR COLLATED
        for sub = 1:3 %numel(R.subname)
            for side = 1:2
                for cond = 1:2
                    if sum(densesave{sub,side}(1,1).N(:))>0 & cond == 1
                        Ai = Ai+1;
                        A(:,:,Ai) = densesave{sub,side}(k,cond).N;
                        Ag(Ai) = sub;
                    elseif sum(densesave{sub,side}(1,2).N(:))>0 & cond == 2
                        Bi = Bi+1;
                        B(:,:,Bi) = densesave{sub,side}(k,cond).N;
                        Bg(Bi) = sub;
                    end
                end
            end
        ...
        statprob = squeeze(stat.prob).*squeeze(stat.mask);
        ...
        % imagesc(x,y,av1');
        caxis([0 0.05]); title(R.condname{1});
        ...
        %     contourf(x,y,N); xlim([-pi pi]); ylim([y(1) y(end)])
        % imagesc(x,y,av2');
        caxis([0 0.05]); title(R.condname{2});
        xlabel('Phi_1 - Phi_2'); ylabel(analynames{k}); set(gca,'YDir','normal');
        h = colorbar; a = ylabel(h, 'Occurence (s^{-1})');
        set(a,'rotation',270); set(a,'Position',get(a,'Position') + [0.8 0 0]);
        subplot(1,3,3);
        ...
        caxis([-3.5 3.5]); title('OFF-ON t-stat'); xlabel('Phi_1 - Phi_2'); ylabel(analynames{k}); h = colorbar; a = ylabel(h, 't-stat');
        set(a,'rotation',270); set(a,'Position',get(a,'Position') + [0.8 0 0]);
        set(gca,'YDir','normal');
        %     pcolortext(statprob')
...
%     savefigure_v2([R.datapathr '\results\seganalysis\PLI\working'],['PLI_PhaseAng_group_seg_analysis_ONvsOFF_MaxCohM1 '],[],[],[]); close all
end
figure(777);
