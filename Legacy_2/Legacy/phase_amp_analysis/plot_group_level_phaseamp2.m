function plot_group_level_phaseamp2(R)
close all
...
%             if subscreen(side,sub)
                for cond = 1:2
                    i = i+1;
                    denseav(i,1,:,:) = [densesave{sub,side}(k,cond).shiftN];
                    design(:,i) = [cond,side,sub]';
                end
%             else
%             end
        end
    end
    A = denseav(design(1,:)==1,1,:,:); A(A==0) = NaN;
    B = denseav(design(1,:)==2,1,:,:); B(B==0) = NaN;
    dimz = [size(densesave{1,side}(k,cond).N) i/2];
...
%         for j = 1:size(A,4)
%             try
%                 [dum pmat(i,j) dum2 stat] = ttest(squeeze(A(:,:,i,j))-squeeze(B(:,:,i,j)));
%                 tmat(i,j) = stat.tstat;
%             catch
%                 pmat(i,j) = NaN;
%                 tmat(i,j) = NaN;
...
%     end
    ...
    stat = clusterstat_matrix(B,A,500,design([1 3],:));
    statmat = squeeze(stat.stat).*squeeze(stat.mask);
    av1 = squeeze(nanmean(B,1));
    av2 = squeeze(nanmean(A,1));
    avdiff = av1-av2;
    ...
    pcolor(x,y,Nbed');
    % imagesc(x,y,av1');
    caxis([0 0.04]); title(R.condname{1});
    ...
    pcolor(x,y,Nbed');
    % imagesc(x,y,av2');
    caxis([0 0.04]); title(R.condname{2});
    xlabel('Phi_1 - Phi_2'); ylabel(analynames{k}); set(gca,'YDir','normal');
    h = colorbar; a = ylabel(h, 'Occurence (s^{-1})');
    set(a,'rotation',270); set(a,'Position',get(a,'Position') + [0.8 0 0]);
    subplot(1,3,3);
    ...
    caxis([-3 3]); %caxis([-0.015 0.015]);
    title('OFF-ON t-stat'); xlabel('Phi_1 - Phi_2'); ylabel(analynames{k}); h = colorbar; a = ylabel(h, 't-stat');
    set(a,'rotation',270); set(a,'Position',get(a,'Position') + [0.8 0 0]);
    set(gca,'YDir','normal')
    clear denseav
...
savefigure_v2([R.datapathr '\results\seganalysis\'],['PhaseAng_group_seg_analysis_ONvsOFF_MaxCohM1 '],[],[],[]); close all
