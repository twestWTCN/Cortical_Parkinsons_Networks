function plot_phase_amp_analysis_PLIs_V3_2side_bars(R)
idd = '';
        ...
        for side =1:2
            for cond = 1:2
                ...
                for nr = 1:nrep
                    ...
                    relativePhi{nr} = wrapToPi(pA_pli_dist_save{cond,nr}); %-circ_mean([pA_pli_dist_save{2,nrOFF}]'));
                    segL{nr} = (segL_pli_dist_save{cond,nr} - mean([segL_pli_dist_save{cond,nr}]'))*100;
                    ampSeg{nr} = amp_pli_dist_save{cond,nr};
                ...
                STN_LB = ampSegCol(3,:);
                STN_LB_Amp = STN_LB(STN_LB>0);
                STN_LB_Phi = relativePhiCol(STN_LB>0)
                ...
                [L4(cond) segLbin(:,nr,cond) binmid pdense(1,cond)] = plot_PA_Dep_relation(cond,relativePhiCol,segLCol,...
                    'Segment Length',QX,linspace(-60,60,QY),f,tendtot); % ,logspace(log10(0.5),log10(1.2),6)
                ...
                [L5(cond) ampbin(:,nr,cond,1) binmid pdense(2,cond)] = plot_PA_Dep_relation(cond,relativePhiCol,ampSegCol(1,:),...
                    'M1 High Beta Amp',QX,linspace(-30,30,QY),f,tendtot); %linspace(0,7,8) ,linspace(-100,200,QY)
                ...
                [L6(cond) ampbin(:,nr,cond,2) binmid pdense(3,cond)] = plot_PA_Dep_relation(cond,relativePhiCol,ampSegCol(2,:),...
                    'STN High Beta Amp',QX,linspace(-30,30,QY),f,tendtot); %linspace(0,7,8) linspace(-100,200,QY)
                xlim([-3.5 3.5]); %ylim([0 8]);
                figure(f(2)); set(gcf,'Position',[263 517 1403 352]);  if cond>0; colorbar; end; title(R.condname{cond})
                caxis([0 0.04]);
                ...
                [L7(cond) ampbin(:,nr,cond,3) binmid pdense(4,cond)] = plot_PA_Dep_relation(cond,relativePhiCol,ampSegCol(3,:),...
                    'STN Low Beta Amp',QX,linspace(-30,30,QY),f,tendtot); % linspace(0,4,8) linspace(-100,200,QY)
