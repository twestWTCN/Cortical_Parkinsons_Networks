...
ax(1) = boundedline(frq,mean(ON,2),std(ON,0,2)./size(ON,2),'cmap',cmap(1,:),'alpha','transparency',0.45); %std(ON,0,2)/sqrt(size(ON,2))
hold on
ax(2) = boundedline(frq,mean(OFF,2),std(OFF,0,2)./size(OFF,2),'cmap',cmap(2,:),'alpha','transparency',0.45);
set(gca,'xscale','log');
set(gca,'yscale','log')
xlim([6 45]);%ylim([0.005 0.05]);
xlabel('Frequency (Hz)');ylabel('Normalized Power'); title(titular{1})
...
ax(1) = boundedline(frq,mean(ON,2),std(ON,0,2)./size(ON,2),'cmap',cmap(1,:),'alpha','transparency',0.45);
hold on
ax(2) = boundedline(frq,mean(OFF,2),std(OFF,0,2)./size(OFF,2),'cmap',cmap(2,:),'alpha','transparency',0.45);
set(gca,'xscale','log'); 
set(gca,'yscale','log')
xlim([6 45]);%ylim([0.0085 0.05]);
xlabel('Frequency (Hz)');ylabel('Normalized Power'); title(titular{2})
ax(1).LineWidth = 1.5; ax(2).LineWidth = 1.5;
legend(ax,{'ON','OFF'}); grid on
...
xlabel('Frequency (Hz)');ylabel('WPLI'); title('STN/CTX WPLI')
ax(1).LineWidth = 1.5; ax(2).LineWidth = 1.5;
legend(ax,{'ON','OFF'})
% set(gca,'xscale','log')
grid on
