...
%     rep
% end
% upperBndEnv = prctile(R_alpha_beta(:),95); upperBnd2 = prctile(R_alpha_beta(:),99);
% lowerBndEnv = prctile(R_alpha_beta(:),5); lowerBnd2 = prctile(R_alpha_beta(:),1);

upperBndEnv = 0.8766;
lowerBndEnv = -0.7857;

b3(2) = plot(tvecAB(:,1),repmat(upperBndEnv,1,size(tvecAB,1)),'b--'); hold on
b3(3) = plot(tvecAB(:,1),repmat(lowerBndEnv,1,size(tvecAB,1)),'b--');

AmpEnvCorr(:,1) = sum(R_alpha_bet_emp>upperBndEnv).*median(diff(tvecAB(:,1)));
AmpEnvCorr(:,2) = sum(R_alpha_bet_emp<lowerBndEnv).*median(diff(tvecAB(:,1)));
...
%     rep
% end
% upperBndPLV = prctile(R_alpha_beta(:),95); upperBnd2 = prctile(R_alpha_beta(:),99);
% lowerBndPLV = prctile(R_alpha_beta(:),5); lowerBnd2 = prctile(R_alpha_beta(:),1);
upperBndPLV = 0.7486;
lowerBndPLV = -0.7193;
b3(5) = plot(tvecAB(:,1),repmat(upperBndPLV,1,size(tvecAB,1)),'k--'); hold on
b3(6) = plot(tvecAB(:,1),repmat(lowerBndPLV,1,size(tvecAB,1)),'k--');
% b2(4) = plot(tvecAB(:,1),repmat(upperBnd2,1,size(tvecAB,1)),'k--'); hold on
% b2(5) = plot(tvecAB(:,1),repmat(lowerBnd2,1,size(tvecAB,1)),'k--');
PLVCorr(:,1) = sum(RPLV_alpha_bet_emp>upperBndEnv).*median(diff(tvecAB(:,1)));
PLVCorr(:,2) = sum(RPLV_alpha_bet_emp<lowerBndEnv).*median(diff(tvecAB(:,1)));
