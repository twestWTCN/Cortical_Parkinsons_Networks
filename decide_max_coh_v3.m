    ...
    for cond = 1:2
        ...
        for nr = nrep
            ...
            for refN = 1:3
                ...
                sub_avg_dics(:,side,refN,nr,cond) = source.avg.coh;
                disp([refN side nr cond])
            end
            end
        end
    end
    % get subject level statistics
    sub_avg_peaks(:,1,1) = max(squeeze(sub_avg_dics(:,1,:,:,1))); % left ON
    sub_avg_peaks(:,2,1) = max(squeeze(sub_avg_dics(:,2,:,:,1))); % right ON
    sub_avg_peaks(:,1,2) = max(squeeze(sub_avg_dics(:,1,:,:,2))); % left OFF
    sub_avg_peaks(:,2,2) = max(squeeze(sub_avg_dics(:,2,:,:,2))); % right OFF
    
    % Get subject average for each side
    sub_avg(:,1) = squeeze(nanmean(nanmean(nanmean(sub_avg_dics(:,1,:,:,:),3),4),5));
    sub_avg(:,2) = squeeze(nanmean(nanmean(nanmean(sub_avg_dics(:,2,:,:,:),3),4),5));
  
    %% Find Left Subject Peak
    % Find distance of each point from reference
    dm = pdist2(source.pos,repmat(premotor_L,size(source.pos,1),1));
    [x i] =min(dm(:,2));
    [max_xyz(1) max_xyz(2) max_xyz(3)] = ind2sub(source.dim,i);
    PM_mask_L = makeSphereMask(source.dim,max_xyz,2);
    PM_mask_L = permute(PM_mask_L,[2 1 3]);
    % Find index of subject peak
    mask =  sub_avg(:,1).*PM_mask_L(:).*(source.pos(:,1)>=0);
    [xi ii] =max(mask(:));
    sub_avg_peak(1) = ii;
    
    %% Find Right Subject Peak
    % Find distance of each point from reference
    dm = pdist2(source.pos,repmat(premotor_R,size(source.pos,1),1));
    [x i] =min(dm(:,2));
    [max_xyz(1) max_xyz(2) max_xyz(3)] = ind2sub(source.dim,i);
    PM_mask_R = makeSphereMask(source.dim,max_xyz,2);
    PM_mask_R = permute(PM_mask_R,[2 1 3]);
    % Find index of subject peak
    mask =  sub_avg(:,2).*PM_mask_R(:).*(source.pos(:,1)<=0);
    [xi ii] =max(mask(:));
    sub_avg_peak(2) = ii;    
  %%%  Potential Plotting Code
%     source.avg.coh = squeeze(sub_avg(:,1));
%     source.coordsys     = 'mni';
...
%         0   0   1.1    0
%         0   0   0    1];
%     TmriT = ft_transform_geometry(T,Tmri);
%     cfg              = [];
%     cfg.parameter    = 'avg.coh';
%     cfg.interpmethod = 'nearest';
%     cfg.coordsys     = 'mni';
%     source_coh_int   = ft_sourceinterpolate(cfg, source, TmriT);
%     cfg = [];
...
%                     0   0   1    2.2
%                     0   0   0    1];
%                 sourceT = ft_transform_geometry(T,source);
%                 cfg            = [];
%                 % cfg.downsample = 1;
%                 cfg.parameter = 'avg.coh';
%                 
%                 sourceInt  = ft_sourceinterpolate(cfg, sourceT , Tmri);
...
%                 [x i] =min(dm(:,2));
%                 [max_xyz(1) max_xyz(2) max_xyz(3)] = ind2sub(source.dim,i);
%                 PM_mask_L = makeSphereMask(source.dim,max_xyz,1.5);
%                 PM_mask_L = permute(PM_mask_L,[2 1 3]);
...
%                 [x i] =min(dm(:,2));
%                 [max_xyz(1) max_xyz(2) max_xyz(3)] = ind2sub(source.dim,i);
%                 PM_mask_R = makeSphereMask(source.dim,max_xyz,1.5);
%                 PM_mask_R = permute(PM_mask_R,[2 1 3]);
...
%     mask = makeSphereMask(GA_sourceDICS_L.dim,max_xyz,1);
...
%     mask = makeSphereMask(GA_sourceDICS_R.dim,max_xyz,1);
