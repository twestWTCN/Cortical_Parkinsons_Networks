...
for i = 2:numel(WinN)
    for bwid = 2:numel(bwidlist)
        for rep = 1:3
        for frqn = 24; %1:numel(frqlist)
            %         figure
            [amp phi dphi_12 dphi_12_dt betaS] = comp_instant_angle_phase(Odata,frqlist(frqn),16,bwidlist(bwid),1026);
            WinSize =WinN(i)*1024;
            ...
            [amp phi dphi_12 dphi_12_dt betaS] = comp_instant_angle_phase(Rdata,frqlist(frqn),16,bwid,1024);
            WinSize = WinN(i)*1024;
            [PLV PLV_tvec] = slidingwindowPLV(WinSize,phi,0.95);
                    figure(1)
            ...
            surL = mean(segL_ddt/1024);
                    figure(2)
                    histogram(segL_ddt/SW_sampr,'FaceColor','r','Normalization','pdf'); hold on
                    figure(3)
