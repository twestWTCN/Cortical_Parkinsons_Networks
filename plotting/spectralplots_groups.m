function [] = spectralplots_groups(R,powsubgrand,cohsubgrand,frq,titular,statflag,subexamp)
if nargin<6
    statflag = 0;
end
...
for i = 3 %1:3
    ...
    [ax(1),hp(1)] = boundedline(frq,mean(OFF,2),std(OFF,0,2)./sqrt(size(OFF,2)),'cmap',cmap(1,:),'alpha','transparency',0.8); %std(ON,0,2)/sqrt(size(ON,2))
    hold on
    [ax(2),hp(2)] = boundedline(frq,mean(ON,2),std(ON,0,2)./sqrt(size(ON,2)),'cmap',cmap(2,:),'alpha','transparency',0.8);
    if subexamp == 1
        delete(hp)
    end
    ...
    legend(ax,R.condname,'Location','NorthEast');grid on
    
    subplot(1,3,2)
    OFF = [powsubgrand{1,2,grouperI{i},:}];
    ON = [powsubgrand{2,2,grouperI{i},:}];
    
    [ax(1),hp(1)] = boundedline(frq,mean(OFF,2),std(OFF,0,2)./sqrt(size(OFF,2)),'cmap',cmap(1,:),'alpha','transparency',0.8);
    hold on
    [ax(2),hp(2)] = boundedline(frq,mean(ON,2),std(ON,0,2)./sqrt(size(ON,2)),'cmap',cmap(2,:),'alpha','transparency',0.8);
