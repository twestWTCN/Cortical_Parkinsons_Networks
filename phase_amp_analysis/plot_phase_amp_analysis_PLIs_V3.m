function plot_phase_amp_analysis_PLIs_V3(R)
idd = '';
        ...
        for cond = 1:2
            ...
            for nr = 1:nrep
                for side =1:2
                    ...
                    relativePhi{side,nr} = wrapToPi(pA_pli_dist_save{cond,nr}); %-circ_mean([pA_pli_dist_save{2,nrOFF}]'));
                    segL{side,nr} = (segL_pli_dist_save{cond,nr} - mean([segL_pli_dist_save{cond,nr}]'))*100;
                    ampSeg{side,nr} = amp_pli_dist_save{cond,nr};
                    HdistSeg{side,nr} = H_dist_save{cond,nr}(1,:);
                    tends(side,nr) = timevec{cond,nr}(end)-timevec{cond,nr}(1);
                end
            end
            tendtot = sum(tends(:));
            ...
            subplot(1,3,cond);
            [L4(cond) segLbin(:,nr,cond) binmid pdense(1,cond)] = plot_PA_Dep_relation(cond,relativePhiCol,segLCol,...
                'Segment Length',QX,-60:10:60,f,tendtot); % ,logspace(log10(0.5),log10(1.2),6)
            ...
            [L5(cond) ampbin(:,nr,cond,1) binmid pdense(2,cond)] = plot_PA_Dep_relation(cond,relativePhiCol,ampSegCol(1,:),...
                'M1 High Beta Amp',QX,-60:10:60,f,tendtot); %linspace(0,7,8) ,linspace(-100,200,QY)
            ...
            [L6(cond) ampbin(:,nr,cond,2) binmid pdense(3,cond)] = plot_PA_Dep_relation(cond,relativePhiCol,ampSegCol(2,:),...
                'STN High Beta Amp',QX,-60:10:60,f,tendtot); %linspace(0,7,8) linspace(-100,200,QY)
            xlim([-3.5 3.5]); %ylim([0 8]);
            figure(f(2)); set(gcf,'Position',[263 517 1403 352]);  if cond>0; colorbar; end; title(R.condname{cond})
            caxis([0 0.04]);
            ...
            [L7(cond) ampbin(:,nr,cond,3) binmid pdense(4,cond)] = plot_PA_Dep_relation(cond,relativePhiCol,ampSegCol(3,:),...
                'STN Low Beta Amp',QX,-30:10:30,f,tendtot); % linspace(0,4,8) linspace(-100,200,QY)
            ...
            [L6(cond) hbin(:,nr,cond,1) binmid pdense(5,cond)] = plot_PA_Dep_relation(cond,relativePhiCol,HdistSegCol,...
                'STN CTX H Beta Amp Corr',QX,linspace(0,1,QY),f,tendtot); % linspace(0,4,8)
            caxis([0 0.08]);            
...
%         savefigure_v2([R.datapathr R.subname{sub} '\images\seganalysis\PLI\'],['PLIseg_analysis_ONvsOFF_MaxCohM1_' R.bandname{band}],[],[],[]);
        close all
