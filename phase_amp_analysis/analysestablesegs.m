function [phi_dist amp_dist seg_ddt segL_ddt consecSegs H] = analysestablesegs(qstable,tseries,refseries,period,mwid,fsamp)
amp = refseries;
...
%         phi_1_dist(j) = circ_mean(wrapTo2Pi(tseries(consecSegs{j},1)));
        amp_dist(1,j) = (max(amp(consecSegs{seglist(j)},1))-median(amp(:,1)))/median(amp(:,1))*100;
        amp_dist(2,j) = (max(amp(consecSegs{seglist(j)},2))-median(amp(:,2)))/median(amp(:,2))*100;
        amp_dist(3,j) = (max(amp(consecSegs{seglist(j)},3))-median(amp(:,3)))/median(amp(:,3))*100;
        % Amp correlations
